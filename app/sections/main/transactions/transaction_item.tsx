...
const TransactionItem: FC<IProps> = ({ transaction }) => {
    ...
    const amountStyles = useMemo(() => cx([
        styles.amount, {plus: transaction.amount > 0}
    ]), [transaction])

    const iconStyles = useMemo(() => cx([
        styles.icon, {isWallet: transaction.type === ETransactionType.BANK_ACCOUNT}
    ]), [transaction])
        ...
        <div className={styles.table_item}>
            <div className={styles.left}>
                <div className={iconStyles}>
                    {transaction.type === ETransactionType.CARD ? <CardIcon /> : <WalletIcon />}
                </div>
                <div>
                    ...
                    <Typography variant="subtitle">
                        ...
                        {/*//@ts-expect-error*/}
                        {moment(transaction.date).format("MMM DD")}, {enumTranslate[transaction.type][i18n.language]}
                    </Typography>
                </div>
            </div>
